...
for (k in Identified_viral_fragments) {
  temp_path = paste0(Path_run_directory,k)
  
  samtools_merging_command = paste("samtools merge",paste0(temp_path,k,"_merge.bam"),
                                   paste(temp_path,"*",sep = ""))
...
for (k in Identified_viral_fragments) {
  ...
  stringtie_command = paste("stringtie ","\'",temp_path,"_merge.bam\'"," -f 0.01 -g 5 ",
                            "-o ","\'",temp_path,"_annotation.gtf\' "," -l \'",k,"\'",sep = "")
...
for (k in Identified_viral_fragments) {
  temp_path = paste0(Path_run_directory,k)
  temp_path = paste(temp_path,"_annotation.gtf",sep = "")
